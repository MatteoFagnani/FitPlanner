...
export default function NewProgramPage() {
      ...
      sessions: [
        {
          ...
          title: "Sessione A",
          order: 1,
          exercises: [{ id: `ex-${Math.random()}`, name: "Squat", sets: 3, reps: 10, method: "RPE 8", percentage: 70, notes: "" }]
        }
      ]
    }
  ]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8 pb-64 relative">
      ...
      {isSyncing && (
        <div className="fixed inset-0 z-[100] bg-background/90 backdrop-blur-xl flex flex-col items-center justify-center space-y-6">
          <div className="w-24 h-24 border-4 border-primary border-t-transparent rounded-full animate-spin glow-sm" />
          <div className="text-center space-y-2">
            <p className="text-2xl font-black uppercase italic tracking-tighter glow-blue">Distribuzione Protocollo</p>
            <p className="text-[10px] font-bold text-outline uppercase tracking-[0.4em] animate-pulse">Sincronizzazione in corso...</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-4 border-b border-outline-variant pb-6">
        ...
        <div className="space-y-1">
          <h2 className="text-3xl font-black uppercase tracking-tighter italic leading-none">Nuovo Protocollo</h2>
          <p className="text-[10px] font-bold text-outline uppercase tracking-widest">Builder Protocollo</p>
        </div>
      </div>

      <form onSubmit={handleCreate} className="space-y-12">
        {/* ASSEGNAZIONE SECTION */}
        <section className="grid md:grid-cols-2 gap-8 bg-white p-8 border-l-4 border-primary shadow-sm">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-outline tracking-[0.2em] flex items-center gap-2">
              <MaterialIcon name="person_search" className="text-sm" /> Atleta
            </label>
            <select
              value={targetAthleteId}
              onChange={(e) => setTargetAthleteId(e.target.value)}
              className="w-full bg-surface-container/50 border-b-2 border-outline-variant px-4 py-3 font-black uppercase italic text-sm focus:border-primary outline-none appearance-none transition-colors"
              required
            >
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-outline tracking-[0.2em] flex items-center gap-2">
              <MaterialIcon name="edit_note" className="text-sm" /> Titolo Protocollo
            </label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full bg-surface-container/50 border-b-2 border-outline-variant px-4 py-3 font-black uppercase italic text-sm focus:border-primary outline-none transition-colors"
            />
          </div>
        </section>
        ...
        <div className="sticky top-16 z-30 space-y-4 bg-background/80 backdrop-blur-md pb-6 pt-2 -mx-4 px-4 border-b border-outline-variant">
           <div className="flex items-center justify-center gap-6">
              ...
              <button 
                type="button" 
                onClick={() => addWeek()} 
                className="w-10 h-10 flex items-center justify-center bg-white hover:bg-primary hover:text-white transition-all border border-outline-variant shadow-sm active:scale-90"
           ...
           <div className="flex items-center justify-between gap-4">
                ...
                {weeks.map((w, idx) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeWeekIdx === idx 
                        ? "bg-primary text-white border-primary shadow-lg glow-sm" 
                        : "bg-white text-outline border-outline-variant hover:border-primary/50"
                    )}
                  >
                    S{idx + 1}
                  </button>
              ...
              <button 
                ...
                className="flex items-center justify-center w-10 h-10 bg-primary text-white shadow-xl hover:bg-blue-600 transition-all shrink-0" 
        ...
        <div className="space-y-20">
          {currentWeek.sessions.map((session, sIdx) => (
            <motion.section 
              ...
              className="space-y-6"
            >
              <div className="flex items-center justify-between bg-surface-container/30 p-4 border border-outline-variant shadow-sm">
                <div className="flex items-center gap-4 flex-1">
                  <div className="w-12 h-12 bg-white flex items-center justify-center font-black italic border border-outline-variant shadow-sm text-primary text-2xl">
                  ...
                  <div className="space-y-1 flex-1">
                    <input
                      ...
                      className="bg-transparent text-2xl font-black uppercase italic tracking-tighter w-full outline-none focus:text-primary transition-colors"
                    />
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center text-outline hover:text-primary transition-colors border border-transparent hover:border-outline-variant"
                  ...
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center text-primary bg-primary/5 hover:bg-primary hover:text-white transition-all"
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-surface-container-low text-[9px] font-black uppercase tracking-[0.2em] text-outline border-b border-outline-variant">
                    <tr>
                      ...
                      <th className="px-6 py-4 min-w-[200px]">Note Strategiche</th>
                      ...
                      >
                        <td className="p-0 border-r border-outline-variant">
                          <input
                            ...
                            className="w-full px-6 py-5 bg-transparent font-black uppercase italic text-sm outline-none focus:text-primary"
                          />
                        </td>
                        <td className="p-0 border-r border-outline-variant">
                          <input
                        ...
                        <td className="p-0 border-r border-outline-variant">
                          <input
                            ...
                            className="w-full px-6 py-5 bg-transparent text-center font-black italic outline-none text-primary"
                            placeholder="--"
                          />
                        </td>
                        <td className="p-0">
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-full max-w-lg px-4 z-40">
           <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-primary text-white py-5 shadow-2xl border border-primary glow-sm flex items-center justify-center gap-4 group"
            >
              <MaterialIcon name="send_and_archive" className="transition-transform group-hover:translate-x-1" />
              <span className="font-black uppercase tracking-[0.4em] text-sm">Distribuisci Protocollo</span>
           </motion.button>
        </div>
      </form>
    </div>
